...
let comidasFavoritas = ["Pizza", "Crema Rusa", "Tortilla", "Hamburguesa", "Café"];
...
comidasFavoritas[0] = "Mila Napolitana";
...
const eterno = ["Doom", "Mirtha", "Topper"];
eterno[2] = "Adidas"; // No se puede 'eterno = ["Adidas"]'
console.log(eterno);
...
eterno.push("Terraria");
eterno[3] = "Minecraft";
console.log(eterno);
// eterno = ["Abigail", "Garras de Bestia", "Terraspark"]
...
const numerosSumar = [5, 5, 3, 4, 3];

function calcularPromedio (datos) {
    let totalNumeros = 0;
    
    for (let A = 0; A < datos.length; A++) {
        totalNumeros = totalNumeros + datos[A];
    }

    console.log(totalNumeros)
}

console.log(calcularPromedio(numerosSumar));
...
for (let A = 0; A < comidasFavoritas.length; A++) {
    console.log(`Me gusta saciarme con ${comidasFavoritas[A]}`)
}
